...
export const getActivePromoBonus = functions.https.onRequest(
  async (req, res) => {
    publicCors(req, res, async () => { // Usar publicCors aquí
      if (req.method !== 'GET') {
  ...
  async (req, res) => {
    authorizeAdmin(req, res, async () => {
      ...
      try {
        ...
        console.error('Error al parsear el cuerpo de la solicitud en addCampaignType:', parseError);
        return res.status(400).json({ message: 'Cuerpo de la solicitud inválido o no JSON.' });
      }

      console.log('Cuerpo recibido para addCampaignType (después del intento de parsing):', newType);

      try {
        const db = getFirestore(getApp());
        // La validación ahora se basa en el 'newType' parseado
        if (!newType || !newType.id || !newType.name) {
          console.log('Fallo de validación: newType:', newType, 'newType.id:', newType.id, 'newType.name:', newType.name);
          return res.status(400).json({
...
export const getCitiesByDepartment = functions.https.onRequest((req, res) => {
  publicCors(req, res, async () => { // Usar publicCors aquí
    ...
    if (!departmentId) {
      return res.status(400).json({
        message: 'Se requiere el ID del departamento como parámetro (departmentId).',
      });
    }
