...
const CampaignTypesSection = ({
  ...
  onAddCampaignType,
  triggerAlert, // Recibir triggerAlert como prop
}) => {
  ...
  const handleAddInputChange = (e) => {
    ...
    setNewCampaignType((prev) => ({ ...prev, [name]: value }))
  }

  const handleAddCampaign = async () => {
    // Función correcta
    if (!newCampaignType.name.trim() || !newCampaignType.description.trim()) {
      triggerAlert(
        'El nombre y la descripción del tipo de campaña no pueden estar vacíos.',
        'warning',
      )
      return
    }
    try {
      await onAddCampaignType(newCampaignType)
      setNewCampaignType({ name: '', description: '', active: true }) // Limpiar formulario
      // La alerta de éxito se maneja en el componente padre
    } catch (error) {
      triggerAlert(`Error al añadir tipo de campaña: ${error.message}`, 'error')
    }
  }
  ...
  const handleSaveEdit = async (item) => {
    ...
    try {
      ...
      triggerAlert(`Error al guardar cambios: ${error.message}`, 'error')
    }
  }
  ...
  const handleDeleteClick = async (id) => {
    // La confirmación y la alerta se manejan en el componente padre
    await onDeleteCampaignType(id)
  }

  const handleToggleActive = async (id, currentStatus) => {
    // La alerta se maneja en el componente padre
    await onToggleCampaignTypeActive(id, !currentStatus)
  }
  ...
  return (
    <section className="bg-white p-3 sm:p-5 rounded-lg shadow-md mb-6 sm:mb-8 border border-neutral-200">
      ...
      <div className="border-t border-neutral-200 pt-4 sm:pt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
          <div>
            ...
            <input
              ...
              name="name"
              value={newCampaignType.name}
              onChange={(e) =>
                setNewCampaignType((prev) => ({
                  ...prev,
                  name: e.target.value,
                }))
              }
          ...
          <div>
            ...
            <textarea
              ...
              name="description"
              value={newCampaignType.description}
              onChange={(e) =>
                setNewCampaignType((prev) => ({
                  ...prev,
        ...
        <button
          onClick={handleAddCampaign} // ¡CORRECCIÓN AQUÍ!
