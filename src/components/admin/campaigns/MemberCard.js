...
const MemberCard = ({ member }) => {
  return (
    <div className="bg-white rounded-xl shadow-sm p-4 flex items-center gap-4 border border-gray-200">
      ...
      <Link
        href={`/dashboard-admin/users/${member.uid}`}
        className="flex-shrink-0 px-4 py-2 text-sm font-medium rounded-md text-blue-600 bg-blue-50 hover:bg-blue-100 transition-colors duration-200"
      >
        Ver Más
      </Link>
    </div>
