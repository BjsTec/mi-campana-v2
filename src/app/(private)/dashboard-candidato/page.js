...
import React, { useState, useEffect, useCallback } from 'react' // Asegúrate de importar todos los hooks que uses
...
// Datos de Colombia (Departamentos y Ciudades con Votos)
const colombiaMapData = {
  ...
  departamentos: [
    {
      id: 'antioquia', // Corresponde a CO-ANT
      name: 'Antioquia',
      votos: 12500,
      promesas: 15000,
      potenciales: 20000,
    ...
    {
      id: 'valle-del-cauca', // Corresponde a CO-VAC
    ...
    {
      id: 'cundinamarca', // Corresponde a CO-CUN
      name: 'Cundinamarca',
      votos: 15000,
      promesas: 20000,
      potenciales: 25000,
    ...
    {
      id: 'atlantico', // Corresponde a CO-ATL
      name: 'Atlántico',
      votos: 8000,
      promesas: 10000,
      potenciales: 14000,
    ...
    {
      id: 'santander', // Corresponde a CO-SAN
      name: 'Santander',
      votos: 7000,
      promesas: 9000,
      potenciales: 11000,
    ...
    {
      id: 'nariño', // Corresponde a CO-NAR
      name: 'Nariño',
      votos: 5000,
      promesas: 6000,
      potenciales: 8000,
    ...
    // Agrega más departamentos si tu SVG tiene IDs diferentes o si quieres añadir más datos.
  ],
}
...
const chartBrandColors = {
  ...
  errorDark: '#dc3545',
  successGreen: '#28a745',
  opinionColors: ['#28a745', '#dc3545', '#ffc107'], // Verde (a favor), Rojo (en contra), Amarillo (indeciso)
}
...
const StatCard = ({ title, value, description, icon: IconComponent }) => (
  <div className="bg-white p-4 sm:p-5 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 flex-1 flex flex-col items-start border border-neutral-200">
    {IconComponent && (
      <IconComponent className="h-6 w-6 sm:h-7 sm:w-7 text-primary-default mb-2" />
    )}
    <h3 className="text-md sm:text-lg font-semibold text-neutral-800 mb-1">
      {title}
    </h3>
    <p className="text-2xl sm:text-3xl font-bold text-primary-default mb-1">
...
const SkeletonCard = () => (
  <div className="bg-white p-4 sm:p-5 rounded-lg shadow-md animate-pulse border border-neutral-200">
    <div className="h-6 w-6 sm:h-7 sm:w-7 bg-neutral-200 rounded-full mb-2"></div>
    <div className="h-3 bg-neutral-200 rounded w-3/4 mb-2"></div>
    <div className="h-6 bg-neutral-300 rounded w-1/2 mb-1"></div>
    <div className="h-3 bg-neutral-200 rounded w-full"></div>
  </div>
)

// --- Contenido del SVG de Colombia (copiado directamente del archivo que proporcionaste) ---
const COLOMBIA_SVG_CONTENT = `<?xml version="1.0" encoding="UTF-8" standalone="no"?>
<svg
...
// Helper para mapear IDs de SVG (CO-XXX) a IDs de datos (snake-case)
const getDepartmentIdFromSvgId = (svgId) => {
  if (!svgId || !svgId.startsWith('CO-')) return null
...
const ColombiaMap = ({ onDepartmentClick, selectedDepartment }) => {
  const handleSvgClick = useCallback(
    (e) => {
      const target = e.target
      // Asciende por el DOM hasta encontrar el path que representa un departamento
      let departmentPath = target
      while (
        departmentPath &&
        departmentPath.tagName !== 'path' &&
        departmentPath.tagName !== 'svg'
      ) {
        departmentPath = departmentPath.parentNode
      }

      if (departmentPath && departmentPath.tagName === 'path') {
        const svgId = departmentPath.id
        const departmentId = getDepartmentIdFromSvgId(svgId)
        if (departmentId) {
  ...
  // Función para procesar el SVG y aplicar estilos
  const getStyledSvg = useCallback(
    (svgContent, selectedId) => {
      // Usar DOMParser para manipular el SVG de forma segura
      ...
      paths.forEach((path) => {
        ...
        const departmentId = getDepartmentIdFromSvgId(svgId)
        if (departmentId) {
          // Colores base de los departamentos (puedes personalizarlos)
          let fillColor = chartBrandColors.primaryLight // Color por defecto

          // Asignar colores diferentes para que se vean distintos si no están seleccionados
          // Puedes usar una lógica más compleja o un array de colores para esto
          if (departmentId === 'antioquia')
            fillColor = '#61A3F7' // Color para Antioquia
          else if (departmentId === 'valle-del-cauca')
            fillColor = '#FCE497' // Color para Valle del Cauca
          else if (departmentId === 'cundinamarca')
            fillColor = '#90EE90' // Light Green para Cundinamarca
          else if (departmentId === 'atlantico')
            fillColor = '#FFB6C1' // Light Pink para Atlántico
          else if (departmentId === 'santander')
            fillColor = '#ADD8E6' // Light Blue para Santander
          else if (departmentId === 'nariño') fillColor = '#DDA0DD' // Plum para Nariño
          // ... añadir más colores para otros departamentos si se desea

          if (departmentId === selectedId) {
            fillColor = chartBrandColors.primaryDefault // Color de selección
          }

          path.setAttribute('fill', fillColor)
          path.setAttribute('stroke', chartBrandColors.primaryDark) // Borde oscuro para todos
          path.setAttribute('stroke-width', '0.5') // Borde más delgado
          // No aplicar clases Tailwind directamente aquí, solo atributos SVG y dejar el hover en CSS global
          // Para hover, puedes definir un CSS global para svg path:hover
        }
      })

      return svgDoc.documentElement.outerHTML // Retorna el SVG modificado como string
    },
    [selectedDepartment],
  ) // Dependencia para re-renderizar si cambia el departamento seleccionado

  const styledSvgContent = getStyledSvg(
    COLOMBIA_SVG_CONTENT,
    selectedDepartment,
  )
...
export default function PanoramaElectoralPage() {
  // === TODOS LOS HOOKS Y VARIABLES DERIVADAS DEBEN DECLARARSE AL PRINCIPIO Y DE FORMA INCONDICIONAL ===
  const { user, isLoading: authLoading } = useAuth()
  const [selectedDepartment, setSelectedDepartment] = useState(null)

  // Los useCallback también son hooks y deben ir al nivel superior
  ...
  // currentDepartment es una variable derivada del estado, debe ir después de useState y antes de retornos condicionales
  ...
  // === AHORA SÍ, LAS COMPROBACIONES CONDICIONALES PUEDEN RETORNAR UI ===
    ...
    datasets: [
      {
        ...
        data: [
          ...
          currentDepartment?.promesas || colombiaMapData.meta.totalPromesas,
        ],
        backgroundColor: [
        ...
        borderColor: [
          chartBrandColors.primaryDark,
          chartBrandColors.primaryDefault,
          chartBrandColors.secondaryDark,
        ],
        borderWidth: 1,
      },
    ],
    ...
    datasets: [
      {
        ...
        borderColor: chartBrandColors.opinionColors.map((color) => color),
        borderWidth: 1,
      },
    ],
  }
  ...
  const chartOptions = {
    responsive: true,
    maintainAspectRatio: false, // Importante para controlar el tamaño de los gráficos
    plugins: {
      legend: {
        position: 'top',
        labels: {
    ...
    scales: {
      x: {
        ...
        grid: {
          color: chartBrandColors.neutral200, // Color más claro para las líneas de la cuadrícula
        },
      },
      y: {
        ticks: {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 bg-neutral-50 min-h-screen">
        ...
        {currentDepartment ? (
          <>
            ...
            >
              ...
              <GlobeAmericasIcon className="h-6 w-6 ml-1" />
            </button>
          ...
          <>
            <GlobeAmericasIcon className="h-7 w-7 text-blue-600" /> Panorama
            Electoral Nacional
          </>
        )}
      </h1>

      {/* Tarjetas de Resumen Global o por Departamento */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard
          title="Votos Totales"
          value={currentDepartment?.votos || colombiaMapData.meta.totalVotos}
          description={
            currentDepartment
              ? `Votos confirmados en ${currentDepartment.name}`
              : 'Votos confirmados a nivel nacional'
          }
          icon={UsersIcon}
        />
        <StatCard
          ...
          description={
            currentDepartment
              ? `Potencial de votos en ${currentDepartment.name}`
              : 'Potencial de votos a nivel nacional'
          }
          icon={ChartBarIcon}
        />
        <StatCard
          ...
          description={
            currentDepartment
              ? `Compromisos de voto en ${currentDepartment.name}`
              : 'Compromisos de voto a nivel nacional'
          }
          icon={HandRaisedIcon}
        />
        {/* Para la opinión, siempre mostraremos la global o una general si no hay datos específicos de departamento */}
        <StatCard
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        {/* Columna para Gráficos Globales/Departamento */}
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white p-4 rounded-lg shadow-md border border-neutral-200">
            <h2 className="text-lg font-semibold text-neutral-800 mb-3">
              Resumen General de Votos
            </h2>
            {/* Se agrega un div contenedor con height para que el gráfico sea responsivo y no se achique demasiado */}
            <div className="relative h-64">
              <Bar
                data={globalVotosData}
                options={{
                  ...chartOptions,
                  plugins: {
                    ...
                    title: {
                      ...
                      text: 'Votos: Totales, Potenciales y Promesas',
                    },
                  },
                  scales: {
                    x: {
                      ...
                      display: true,
                      title: { display: true, text: 'Tipo de Voto' },
                    },
                    y: {
      ...
      {/* Sección de Ciudades del Departamento Seleccionado */}
      {currentDepartment && (
        <div className="bg-white p-6 rounded-lg shadow-md border border-neutral-200 mt-8">
          <h2 className="text-xl font-semibold text-neutral-800 mb-4">
            Ciudades en {currentDepartment.name}
          </h2>
          {currentDepartment.ciudades.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-neutral-200 text-sm">
                ...
                <tbody className="bg-white divide-y divide-neutral-200">
                  {currentDepartment.ciudades.map((ciudad, index) => (
                    <tr
                      key={index}
