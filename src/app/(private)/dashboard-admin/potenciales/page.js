...
const LeadCard = ({ lead }) => {
  ...
  return (
    <div className="bg-neutral-50 p-4 rounded-lg shadow-md border border-neutral-200 flex flex-col justify-between transform hover:scale-[1.02] transition-transform duration-200 ease-in-out">
      {' '}
      {/* Changed background to neutral-50 */}
      <div>
  ...
  useEffect(() => {
    const fetchLeads = async () => {
      if (authLoading || !idToken) {
        ...
        if (!authLoading && !idToken) {
          setError(
            'Not authenticated. Please log in to view potential customers.',
          )
          setLoading(false)
        }
        return
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.message || 'Error fetching potential customers.',
          )
        }
  ...
  const filterLeads = (leads) => {
    ...
    return leads.filter(
      (lead) =>
        ...
        (lead.source &&
          lead.source.toLowerCase().includes(lowerCaseSearchTerm)) || // Added filter by source
        (lead.status &&
          lead.status.toLowerCase().includes(lowerCaseSearchTerm)), // Added filter by status
    )
  }
      ...
      <div className="container mx-auto">
        ...
        {!loading && !error && displayedLeads.length === 0 && (
          <p className="text-center text-neutral-600 text-lg py-10">
            No potential customers found in this list.
          </p>
        )}
