...
export default function LeadDetailPage() {
  ...
  const fetchLead = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`https://us-central1-micampanav2.cloudfunctions.net/getLeadById?id=${leadId}`, {
        method: 'GET',
        headers: {
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      const response = await fetch('https://us-central1-micampanav2.cloudfunctions.net/updateLead', {
    ...
    return (
      <div className="p-8 bg-neutral-100 min-h-screen flex items-center justify-center">
        <div className="bg-error text-white p-4 rounded-md shadow-md">
          <p>{error}</p>
          <button onClick={() => router.back()} className="mt-4 bg-white text-error px-4 py-2 rounded-md hover:bg-neutral-100">Volver</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="p-8 bg-neutral-100 min-h-screen flex items-center justify-center">
        <div className="bg-neutral-50 text-neutral-800 p-6 rounded-md shadow-md">
          ...
          <button onClick={() => router.back()} className="mt-4 bg-primary-DEFAULT text-neutral-50 px-4 py-2 rounded-md hover:bg-primary-dark">Volver</button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-neutral-100 p-8">
      <div className="container mx-auto max-w-4xl bg-white rounded-lg shadow-xl p-8">
        ...
        <div className="flex justify-between items-center mb-6 border-b pb-4">
          <h1 className="text-3xl font-bold text-primary-dark">Perfil de Cliente Potencial</h1>
          <Link href="/dashboard-admin/potenciales" className="inline-flex items-center bg-neutral-200 text-neutral-800 px-4 py-2 rounded-full text-sm font-semibold hover:bg-neutral-300 transition-colors duration-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <div>
            ...
            <p className="text-primary-dark text-lg font-medium">{lead.email}</p>
          </div>
          <div>
            <p className="text-neutral-600 text-sm font-semibold">Teléfono:</p>
            <p className="text-primary-dark text-lg font-medium">{lead.phone || 'N/A'}</p>
          </div>
          <div>
            <p className="text-neutral-600 text-sm font-semibold">Interesado en:</p>
            <p className="text-primary-dark text-lg font-medium">{lead.interestedIn || 'N/A'}</p>
          </div>
          <div>
            <p className="text-neutral-600 text-sm font-semibold">Fuente:</p>
            <p className="text-primary-dark text-lg font-medium">{lead.source || 'N/A'}</p>
          </div>
          <div>
            <p className="text-neutral-600 text-sm font-semibold">Recibido:</p>
            <p className="text-primary-dark text-lg font-medium">{formatTimestamp(lead.timestamp)}</p>
          </div>
        </div>
        ...
        <div className="mb-8 p-6 bg-neutral-50 rounded-lg border border-neutral-200 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-semibold text-primary-dark">Estado Actual: 
          ...
          {isEditingStatus && (
            <div className="flex items-center gap-4 mt-4">
              ...
              >
                {updateStatus === 'loading' ? 'Guardando...' : <><SaveIcon /> Guardar</>}
              </button>
              <button 
          ...
          {updateStatus === 'success' && <p className="text-success text-sm mt-2">¡Estado actualizado!</p>}
          {updateStatus === 'error' && <p className="text-error text-sm mt-2">Error al actualizar estado.</p>}
        </div>

        {/* Sección de Notas */}
        <div className="mb-8 p-6 bg-neutral-50 rounded-lg border border-neutral-200 shadow-sm">
          <h3 className="text-xl font-semibold text-primary-dark mb-4">Historial de Notas</h3>
          <div className="max-h-60 overflow-y-auto border border-neutral-200 rounded-md p-3 bg-white mb-4">
            {lead.notes && lead.notes.length > 0 ? (
              lead.notes.map((note, index) => (
                <div key={index} className="mb-3 pb-3 border-b border-neutral-100 last:border-b-0">
                  <p className="text-neutral-800 text-sm">{note.text}</p>
                  <p className="text-neutral-500 text-xs mt-1">
                    {formatTimestamp(note.timestamp)} por {note.adminId || 'Admin'}
                    {note.type === 'status_change' && <span className="ml-2 px-2 py-0.5 bg-blue-100 text-blue-800 rounded-full text-xs">Cambio de Estado</span>}
                  </p>
                </div>
              ))
            ) : (
              <p className="text-neutral-600 text-sm">No hay notas registradas para este cliente potencial.</p>
            )}
          </div>

          <div className="mt-4">
            <label htmlFor="newNote" className="block text-neutral-800 text-sm font-bold mb-2">Añadir Nueva Nota</label>
            <textarea
              id="newNote"
              value={newNote}
            ...
            >
              <AddNoteIcon /> {updateStatus === 'loading' ? 'Añadiendo...' : 'Añadir Nota'}
            </button>
          </div>
        </div>
      </div>
