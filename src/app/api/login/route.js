...
export async function POST(request) {
  try {
    ...
    if (!email || !password) {
      return new Response(JSON.stringify({ error: 'Faltan credenciales (cédula o contraseña).' }), {
    ...
    const firebaseFunctionUrl = 'https://us-central1-micampanav2.cloudfunctions.net/loginWithEmail';
    ...
    if (!firebaseResponse.ok) {
      ...
      return new Response(JSON.stringify({ error: firebaseDaCta.message || firebaseDaCta.error || 'Error al conectar con la función de autenticación.' }), {
